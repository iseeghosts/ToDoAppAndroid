...
export default class SavedNote extends Component{
    ...
    constructor (props) {
        super()
            this.state = ({
                ...
                backColor:(props.task.content==''? '#98817B':(props.task.status!="Complete"? 'lightgrey':'skyblue')),      // 
    ...
    delete_task = () => {
        if (this.state.task) {
            ...
            Alert.alert('Delete?', "Are you sure you want to delete the task " + this.state.task + '?'
            ,[
                {text:'Confirm', onPress: () => {this.setState({'deletedTask':'none'}); delete this.props.task.content; delete this.props.task.id;}},
